...
class k_means:
    ...
    def plot_cross_validation(self):
        ...
        for n in range(1, len(self.class_list) + 1):
            ...
            num_clusters = n
            k_means = joshkmeans()
            scores = cross_val_score(k_means, self.x_train, self.y_train, cv=k_fold_values[self.dataset],
                                     scoring='accuracy')
        ...
        #     'Number of Clusters',
        #     'Mean Accuracy Score',
        #     'KMeans - clusters vs accuracy - dataset : ' + self.dataset,
        #     'linear'
        # )
...
def test_kmeans(dataset):
    ...
    for train_percent in train_percentage:
        ...
        accuracy_values.append(learn_kmeans.checkValidation())
